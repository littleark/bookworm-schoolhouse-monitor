...
export function BookDetail({ studentBook, studentName, onBack }: BookDetailProps) {
  const totalTimeSpent = studentBook.sessions.reduce((total, session) => total + session.timeSpent, 0);
  const totalPagesRead = studentBook.sessions.reduce((total, session) => total + session.pagesRead, 0);
  const averageSessionTime = studentBook.sessions.length > 0 ? totalTimeSpent / studentBook.sessions.length : 0;
        ...
        <CardHeader>
          <div className="flex gap-6">
            ...
            <div className="flex-1 space-y-3">
              <div>
                <CardTitle className="text-xl">{studentBook.book.title}</CardTitle>
                <p className="text-muted-foreground">by {studentBook.book.author}</p>
                <p className="text-sm text-muted-foreground">Reading progress for {studentName}</p>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            ...
            <p className="text-2xl font-bold">{Math.round(averageSessionTime)}</p>
      ...
      <Card>
        ...
        <CardContent>
          {studentBook.sessions.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">No reading sessions recorded yet</p>
          ) : (
